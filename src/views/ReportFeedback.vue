<template>
  <defaultLayout>
    <section class="mx-auto mb-auto">
      ...
      <div class="max-w-xl mt-4 mx-auto p-6 bg-base-200 rounded-lg shadow-lg fadeRight w-full">
        <form @submit.prevent="submit" class="">
          <MCInput textIcon="mdi:format-title" textLabel="Titulo" :textError="title.errorMessage.value">
            <input v-model="title.value.value" id="title" placeholder="Elige un titulo descriptivo ... "
              class="input input-bordered w-full mt-1 block w-full py-2 px-3 rounded-md shadow-sm "></input>
          </MCInput>
          ...
          <MCInput textIcon="mdi:text-box" textLabel="Descripcion" :textError="description.errorMessage.value">
            <textarea v-model="description.value.value" id="description"
              placeholder="Describe el error/ la nueva idea ..."
              class="textarea textarea-bordered w-full mt-1 block w-full py-2 px-3 rounded-md shadow-sm h-32 "></textarea>
          </MCInput>
